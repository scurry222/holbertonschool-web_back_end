...
app.get('/reserve_seat', async (req, res) => {
    if (!reservationEnabled) {
        res.status(403).send(
            JSON.stringify({ status: 'Reservations are blocked' })
        );
        return;
    }
    ...
    const newJob = queue.create('reserve_seat', {}).save((err) => {
        if (err) {
            res.status(500).send(
                JSON.stringify({ status: 'Reservations failed' })
            );
            return;
        }
    ...
    newJob.on('failed', (err) => console.log(
        `Seat reservation job ${newJob.id} failed: ${err}`
    ))

    newJob.on('complete', () => console.log(
        `Seat reservation job ${newJob.id} completed`
    ))
})

app.get('/process', (req, res) => {
    process().then(() => res.send(
        JSON.stringify({
